...
<head>
    ...
    <title>Profil de l'Apprenant - BJ Academie</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar" class="sidebar-collapsed sidebar-bg text-white flex flex-col py-6">
            ...
            <nav class="flex-1 flex flex-col gap-6 w-full px-2">
                @php
                    ...
                    $isApprenants = $currentRoute === 'formateur.apprenants' || $currentRoute === 'formateur.apprenant.profil';
                @endphp
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-gray-50">
            ...
            <header class="bg-white border-b border-gray-200 px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('formateur.apprenants') }}" class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors" title="Retour">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                            </svg>
                        </a>
                        <h1 class="text-xl font-semibold text-gray-900">Profil de l'Apprenant</h1>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto">
                <!-- Cover Image Section -->
                <div class="cover-image h-64 relative"></div>
                    ...
                    <div class="max-w-6xl mx-auto px-6 pb-6">
                        <div class="flex items-end gap-6 -mt-20 relative z-10">
                            ...
                            <div class="flex-1 pb-4">
                                <h2 class="text-3xl font-bold text-gray-900 mb-2">{{ $apprenant->prenom ?? '' }} {{ $apprenant->nom ?? '' }}</h2>
                                <div class="flex flex-wrap items-center gap-6 text-gray-600">
                                    <div class="flex items-center gap-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                                        </svg>
                                        <span class="text-sm">Apprenant</span>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm">{{ $apprenant->filiere }}</span>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm">{{ ucfirst(str_replace('_', ' ', $apprenant->classe_id)) }}</span>
                                    </div>
                                    @endif
                                    <div class="flex items-center gap-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <span class="text-sm">Membre depuis {{ $apprenant->created_at->format('F Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="max-w-6xl mx-auto px-6 py-8">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                        ...
                        <p class="text-gray-600 mb-6">Informations de l'apprenant</p>

                        <form class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Prénom</label>
                                    <input type="text" value="{{ $apprenant->prenom ?? '' }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Entrer le prénom" readonly>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Nom</label>
                                    <input type="text" value="{{ $apprenant->nom ?? '' }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Entrer le nom" readonly>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Email</label>
                                    <input type="email" value="{{ $apprenant->email ?? '' }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Entrer l'email" readonly>
                                </div>
                                @if($apprenant->filiere)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Filière</label>
                                    <input type="text" value="{{ $apprenant->filiere }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" readonly>
                                </div>
                                @endif
                                @if($apprenant->niveau_etude)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Niveau d'étude</label>
                                    <input type="text" value="{{ $apprenant->niveau_etude }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" readonly>
                                </div>
                                @endif
                                @if($apprenant->nationalite)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Nationalité</label>
                                    <input type="text" value="{{ $apprenant->nationalite }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" readonly>
                                </div>
                                @endif
                            </div>

                            <div class="pt-4">
                                <a href="{{ route('formateur.apprenants') }}" class="inline-flex items-center px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium">
                                    Retour à la liste
                                </a>
                            </div>
                        </form>
