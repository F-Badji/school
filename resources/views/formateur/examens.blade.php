...
<head>
    ...
    <title>Programmer des examens - BJ Academie</title>
...
<body>
            ...
            <header class="bg-white border-b border-gray-200 px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex-1 max-w-md">
                        <div class="relative">
                            <input type="text" id="searchInput" placeholder="Rechercher un examen..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                    ...
                    <div class="flex-1 flex items-center justify-center">
                        <nav class="flex items-center space-x-2 text-sm text-gray-600">
                            ...
                            <span class="text-gray-900 font-medium">Programmer des examens</span>
                        </nav>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto bg-gray-50">
                <div class="max-w-7xl mx-auto px-6 py-8">
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 mb-2">Programmer des examens</h1>
                            <p class="text-gray-600">Gérez tous les examens des étudiants en un seul endroit</p>
                        </div>
                        <a href="{{ route('formateur.examens.create') }}" class="px-6 py-3 text-white rounded-lg transition-all shadow-lg hover:shadow-xl font-medium flex items-center gap-2" style="background: linear-gradient(180deg, #1a1f3a 0%, #161b33 100%);">
                    ...
                    <!-- Examens Grid -->
                    @if($examens->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="examensGrid">
                                ...
                                <div class="course-card bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('formateur.examens.edit', $examen->id) }}" class="flex-1 px-4 py-2 text-white rounded-lg transition-colors text-center text-sm font-medium" style="background: linear-gradient(180deg, #1a1f3a 0%, #161b33 100%);" onmouseover="this.style.background='linear-gradient(180deg, #161b33 0%, #1a1f3a 100%)'" onmouseout="this.style.background='linear-gradient(180deg, #1a1f3a 0%, #161b33 100%)'">
                                                Modifier
                                            </a>
                                            <form action="{{ route('formateur.examens.destroy', $examen->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet examen ?');">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @else
                        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
                            ...
                            <h3 class="text-xl font-semibold text-gray-900 mb-2">Aucun examen pour le moment</h3>
                            <p class="text-gray-600 mb-6">Commencez par créer votre premier examen</p>
                            <a href="{{ route('formateur.examens.create') }}" class="inline-flex items-center gap-2 px-6 py-3 text-white rounded-lg transition-all shadow-lg hover:shadow-xl font-medium" style="background: linear-gradient(180deg, #1a1f3a 0%, #161b33 100%);">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                </svg>
                    ...
                    <!-- Section Réponses des examens -->
                    @if($examens->count() > 0)
                        <div class="mt-12">
                            <div class="mb-6">
                                <h2 class="text-2xl font-bold text-gray-900 mb-2">Réponses des examens</h2>
                                <p class="text-gray-600">Consultez et évaluez les réponses soumises par vos apprenants pour chaque examen. Cette section vous permet de suivre en détail les questions posées et les réponses fournies, facilitant ainsi l'évaluation et le suivi pédagogique.</p>
                            </div>
                                    ...
                                    @if($examen->questions->count() > 0)
                                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                            ...
                                            <div class="bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
                                                <div class="flex items-center justify-between">
                                                    <div>
                                                        <h3 class="text-lg font-bold text-gray-900">{{ $examen->titre }}</h3>
                                                        <p class="text-sm text-gray-600 mt-1">
                                                            {{ $examen->matiere->nom_matiere ?? 'Matière non spécifiée' }}
                                                            @if($examen->date_examen)
                                                                • {{ \Carbon\Carbon::parse($examen->date_examen)->format('d/m/Y') }}
                                                            @endif
                                                            @if($examen->duree_minutes)
                                                                • Durée : {{ $examen->duree_minutes }} minutes
                                                            @endif
                                                        </p>
                                                    </div>
                                                    <div class="text-right">
                                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $examen->actif ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                    ...
                                                    @foreach($examen->questions as $question)
                                                        <div class="border border-gray-200 rounded-lg p-5 bg-gray-50">
                                                                ...
                                                                <div class="flex items-start justify-between mb-3">
                                                                    <div class="flex-1">
                                                                        <div class="flex items-center gap-2 mb-2">
                                                                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-purple-600 text-white text-sm font-bold">
                                                                                {{ $loop->iteration }}
                                                                            </span>
                                                            ...
                                                            <div class="mt-4 pt-4 border-t border-gray-300">
                                                                ...
                                                                @php
                                                                    $reponsesQuestion = $examen->reponses->where('examen_question_id', $question->id)->groupBy('user_id');
                                                                @endphp
                                                                
                                                                @if($reponsesQuestion->count() > 0)
                                                                    <div class="space-y-4">
                                                                        @foreach($reponsesQuestion as $userId => $reponses)
                                                                            @php
                                                                                $apprenant = $reponses->first()->apprenant ?? null;
                                                                                ...
                                                                                <div class="bg-white rounded-lg border border-gray-200 p-4">
                                                                                    <div class="flex items-start justify-between mb-3">
                                                                                        ...
                                                                                        <span class="text-xs text-gray-500">
                                                                                            {{ $reponse->soumis_le ? \Carbon\Carbon::parse($reponse->soumis_le)->format('d/m/Y à H:i') : 'Non daté' }}
                                                                                        </span>
                                                                                    </div>
                                                                ...
                                                                @else
                                                                    <div class="bg-gray-50 rounded-lg p-4 text-center border border-gray-200">
                                                                        <p class="text-sm text-gray-500 italic">Aucune réponse soumise pour cette question pour le moment</p>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const examensGrid = document.getElementById('examensGrid');
            
            if (searchInput && examensGrid) {
                searchInput.addEventListener('input', function(e) {
                    const searchTerm = e.target.value.toLowerCase();
                    const examenCards = examensGrid.querySelectorAll('.course-card');
                    
                    examenCards.forEach(card => {
