...
<head>
    ...
    <title>{{ isset($examen) ? 'Modifier l\'examen' : 'Nouvel examen' }} - BJ Academie</title>
...
<body>
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-gray-50">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex-1 flex items-center">
                        <nav class="flex items-center space-x-2 text-sm text-gray-600">
                            ...
                            <a href="{{ route('formateur.examens') }}" class="hover:text-gray-900">Mes examens</a>
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                            </svg>
                            <span class="text-gray-900 font-medium">{{ isset($examen) ? 'Modifier' : 'Nouvel examen' }}</span>
                        </nav>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto bg-white">
                <div class="max-w-7xl mx-auto px-6 py-6">
                    <!-- Page Title -->
                    <div class="mb-6">
                        <h1 class="text-3xl font-bold text-gray-900">{{ isset($examen) ? 'Modifier l\'examen' : 'Nouvel examen' }}</h1>
                    </div>
                    
                    <form action="{{ isset($examen) ? route('formateur.examens.update', $examen->id) : route('formateur.examens.store') }}" method="POST" id="examenForm" enctype="multipart/form-data">
                        @csrf
                        @if(isset($examen))
                            @method('PUT')
                        @endif
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <div class="flex items-center justify-end mb-6">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('formateur.examens') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                                            Annuler
                                        </a>
                                ...
                                <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
                                    ...
                                    <!-- Title Field -->
                                    <div class="mb-6">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Titre de l'examen *</label>
                                        <input type="text" name="titre" value="{{ old('titre', isset($examen) ? ($examen->titre ?? '') : '') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Ex: Examen final - Algorithmes">
                                        @error('titre')
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <textarea name="description" rows="4" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Description de l'examen...">{{ old('description', isset($examen) ? ($examen->description ?? '') : '') }}</textarea>
                                        @error('description')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <select name="matiere_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            ...
                                            @foreach($matieres as $matiere)
                                                <option value="{{ $matiere->id }}" {{ old('matiere_id', isset($examen) ? ($examen->matiere_id ?? '') : '') == $matiere->id ? 'selected' : '' }}>{{ $matiere->nom_matiere }}</option>
                                            @endforeach
                                        </select>
                                        @error('matiere_id')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    ...
                                    <!-- Date examen Field -->
                                    <div class="mb-6">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Date de l'examen</label>
                                        <input type="date" name="date_examen" value="{{ old('date_examen', isset($examen) && $examen->date_examen ? $examen->date_examen->format('Y-m-d') : '') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                        @error('date_examen')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 mb-6">
                                        <div>
                                            ...
                                            <input type="time" name="heure_debut" value="{{ old('heure_debut', isset($examen) ? ($examen->heure_debut ?? '') : '') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            @error('heure_debut')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div>
                                            ...
                                            <input type="time" name="heure_fin" value="{{ old('heure_fin', isset($examen) ? ($examen->heure_fin ?? '') : '') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            @error('heure_fin')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <!-- Durée minutes Field -->
                                    <div class="mb-6">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Durée (en minutes)</label>
                                        <input type="number" name="duree_minutes" value="{{ old('duree_minutes', isset($examen) ? ($examen->duree_minutes ?? '') : '') }}" min="1" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Ex: 120">
                                        @error('duree_minutes')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <input type="number" name="points_total" value="{{ old('points_total', isset($examen) ? ($examen->points_total ?? 20) : 20) }}" min="1" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="20">
                                        @error('points_total')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
                                    ...
                                    <p class="text-sm text-gray-600 mb-4">Définissez un code à 6 chiffres pour sécuriser l'interface de l'examen. Les apprenants devront entrer ce code pour déverrouiller l'interface pendant l'examen.</p>
                                    
                                    <div class="mb-4">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Code de sécurité (6 chiffres) *</label>
                                        <div class="flex items-center gap-3">
                                            <input type="text" name="code_securite" id="codeSecurite" value="{{ old('code_securite', isset($examen) ? ($examen->code_securite ?? '') : '') }}" maxlength="6" pattern="[0-9]{6}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-center text-2xl font-mono tracking-widest" placeholder="000000">
                                ...
                                <div class="bg-white rounded-lg border border-gray-200 p-6">
                                    <div class="flex items-center justify-between mb-4">
                                        <h2 class="text-lg font-semibold text-gray-900">Questions de l'examen</h2>
                                    ...
                                    <div id="questions-container" class="space-y-3">
                                        @if(isset($examen) && isset($examen->questions) && $examen->questions->count() > 0)
                                            @foreach($examen->questions as $question)
                                                @include('formateur.partials.examen-question-item', ['question' => $question, 'questionIndex' => $loop->index])
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <div class="bg-white rounded-lg border border-gray-200 p-6">
                                        ...
                                        <div class="space-y-3 text-sm">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span class="text-gray-900 font-medium">{{ isset($examen) ? $examen->created_at->format('d/m/Y') : 'Nouveau' }}</span>
                                            </div>
                                            @if(isset($examen))
                                                <div class="flex items-center justify-between">
                                                    <span class="text-gray-600">Modifié le:</span>
                                                    <span class="text-gray-900 font-medium">{{ $examen->updated_at->format('d/m/Y') }}</span>
                                                </div>
                                                <div class="flex items-center justify-between">
                                                    <span class="text-gray-600">Questions:</span>
                                                    <span class="text-gray-900 font-medium">{{ isset($examen) && $examen->questions ? $examen->questions->count() : 0 }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
    ...
    <script>
        ...
        // Question Management
        let questionCounter = {{ isset($examen) && isset($examen->questions) ? $examen->questions->count() : 0 }};
