...
<head>
    ...
    <title>Mes Devoirs - BJ Academie</title>
    ...
    <style>
        * {
        ...
        .devoir-card .h-48 img {
            transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1);
            transform: scale(1);
        }
        .devoir-card:hover .h-48 img {
            transform: scale(1.2);
        }
...
<body>
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-gray-50">
            @php
                $devoirsSoumis = [];
                if($tentativesSoumises && $tentativesSoumises->count() > 0 && $devoirs) {
                    foreach($devoirs as $devoir) {
                        if(isset($tentativesSoumises[$devoir->id]) && $tentativesSoumises[$devoir->id]->soumis_le) {
                            $devoirsSoumis[] = [
                                'devoir' => $devoir,
                                'tentative' => $tentativesSoumises[$devoir->id]
                            ];
                        }
                    }
                }
            ...
            <!-- Alertes fixes pour devoirs soumis - En haut de la page -->
            @if(count($devoirsSoumis) > 0)
                <div class="fixed-alert-container">
                    <div class="max-w-7xl mx-auto px-8">
                        <div class="space-y-1.5">
                            @foreach($devoirsSoumis as $item)
                                <div class="flex items-center justify-center gap-2.5 text-sm">
                                    ...
                                    <span class="text-gray-700 text-center">
                                        Vous avez terminé le devoir <span class="font-semibold text-gray-900">"{{ $item['devoir']->titre }}"</span> à <span class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($item['tentative']->soumis_le)->format('H:i') }}</span>
                                    </span>
                                </div>
                            @endforeach
                        </div>
            ...
            <header class="bg-white border-b border-gray-200 px-6 py-4" style="position: relative; z-index: 999;">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-600 text-sm mt-1">Gérez et consultez tous vos devoirs</p>
                    </div>
            ...
            <!-- Main Content Area -->
            <div class="flex-1 overflow-y-auto bg-gray-50 p-8" style="padding-top: {{ count($devoirsSoumis) > 0 ? '140px' : '32px' }};">
                <div class="max-w-7xl mx-auto">
                    ...
                    @endif
                    @if($devoirs && $devoirs->count() > 0)
                        <!-- Devoirs Grid -->
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($devoirs as $devoir)
                                @php
                                    $matiereNom = $devoir->matiere->nom_matiere ?? 'Devoir';
                                    $formateurNom = ($devoir->formateur->prenom ?? '') . ' ' . ($devoir->formateur->nom ?? '');
                                    $tentativeSoumise = $tentativesSoumises[$devoir->id] ?? null;
                                    $devoirSoumis = $tentativeSoumise !== null;
                                    
                                    // Vérifier si la date/heure programmée est dépassée
                                    $tempsEcoule = false;
                                    if ($devoir->date_devoir && $devoir->heure_fin) {
                                        $dateDevoirOnly = \Carbon\Carbon::parse($devoir->date_devoir)->format('Y-m-d');
                                        $heureFin = \Carbon\Carbon::parse($dateDevoirOnly . ' ' . $devoir->heure_fin);
                                        $now = \Carbon\Carbon::now();
                                        $tempsEcoule = $now->gt($heureFin);
                                    }
                                    
                                    // Si le temps est écoulé et le devoir n'est pas soumis, considérer comme terminé
                                    $devoirTermine = $devoirSoumis || ($tempsEcoule && !$devoirSoumis);
                                @endphp
                                <div class="devoir-card bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                    <!-- Image de couverture -->
                                    <div class="h-48 relative overflow-hidden">
                                        @if($devoir->image_couverture)
                                            <img src="{{ asset('storage/' . $devoir->image_couverture) }}" alt="{{ $devoir->titre }}" class="w-full h-full object-cover">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <!-- Alerte si le temps est écoulé -->
                                        @if($tempsEcoule && !$devoirSoumis)
                                            <div class="mb-4 bg-orange-50 border border-orange-200 text-orange-800 px-4 py-3 rounded-lg flex items-center gap-2">
                                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                </svg>
                                                <span class="text-sm">Le temps imparti est écoulé. Votre devoir a été soumis automatiquement.</span>
                                            </div>
                                        @endif
                                        ...
                                        @else
                                            <button onclick="requestCodeBeforeStart({{ $devoir->id }})" class="block w-full px-4 py-2.5 text-white rounded-lg text-sm font-medium transition-colors text-center hover:opacity-90" style="background: linear-gradient(180deg, #1a1f3a 0%, #161b33 100%);">
                                                Commencer le devoir
                                            </button>
                                        @endif
                                    </div>
                    ...
                    @else
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                            <div class="text-center py-12">
                                ...
                                <h2 class="text-2xl font-bold text-gray-900 mb-2">Aucun devoir disponible</h2>
                                <p class="text-gray-600">Vous n'avez pas encore de devoirs assignés par vos professeurs.</p>
                            </div>
                        </div>
                    @endif
                </div>
    ...
    <script>
        ...
        console.log('🔍 [DIAGNOSTIC DEVOIRS] Script chargé');
        
        // Profile dropdown menu
        document.addEventListener('DOMContentLoaded', function() {
            console.log('🔍 [DIAGNOSTIC DEVOIRS] DOMContentLoaded déclenché');
            
            const parametresBtn = document.getElementById('parametresBtn');
            console.log('🔍 [DIAGNOSTIC DEVOIRS] parametresBtn trouvé:', parametresBtn);
            
            if (parametresBtn) {
                console.log('🔍 [DIAGNOSTIC DEVOIRS] href du bouton:', parametresBtn.getAttribute('href'));
                
                parametresBtn.addEventListener('click', function(e) {
                    console.log('🔍 [DIAGNOSTIC DEVOIRS] Clic détecté via addEventListener');
                    console.log('🔍 [DIAGNOSTIC DEVOIRS] Event:', e);
                    console.log('🔍 [DIAGNOSTIC DEVOIRS] DefaultPrevented:', e.defaultPrevented);
                    console.log('🔍 [DIAGNOSTIC DEVOIRS] href:', this.getAttribute('href'));
                }, false);
            } else {
                console.error('❌ [DIAGNOSTIC DEVOIRS] parametresBtn introuvable!');
            }
        ...
        // Modal pour demander le code avant de commencer le devoir
        let currentDevoirId = null;

        function requestCodeBeforeStart(devoirId) {
        ...
        function verifyCodeAndStart() {
            ...
            fetch('{{ route("apprenant.devoir.unlock", ":id") }}'.replace(':id', currentDevoirId), {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    // Code correct, rediriger vers le devoir
                    window.location.href = '{{ route("apprenant.devoir.passer", ":id") }}'.replace(':id', currentDevoirId);
                } else {
    ...
    <div id="codeModal" class="fixed inset-0 bg-black bg-opacity-80 backdrop-blur-sm z-[10000] flex items-center justify-center hidden">
        <div class="bg-white rounded-xl p-8 max-w-md w-full mx-4">
            <h3 class="text-2xl font-bold text-gray-900 mb-4">Code de sécurité requis</h3>
            <p class="text-gray-600 mb-6">Pour commencer ce devoir, veuillez entrer le code de sécurité fourni par votre professeur.</p>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Code de sécurité (6 chiffres)</label>
                <input type="text" id="codeInput" maxlength="6" autocomplete="off" autofill="off" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-center text-2xl font-mono tracking-widest" placeholder="000000">
            </div>
            <div id="codeMessage" class="mb-4 text-sm"></div>
            <div class="flex gap-3 mt-8">
                <button onclick="closeCodeModal()" class="flex-1 px-4 py-2 text-sm border-2 border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                    Annuler
                </button>
                <button onclick="verifyCodeAndStart()" class="flex-1 px-4 py-2 text-sm text-white rounded-lg font-medium hover:opacity-90 transition-colors" style="background: linear-gradient(180deg, #1a1f3a 0%, #161b33 100%);">
                    Démarrer
                </button>
            </div>
        </div>
