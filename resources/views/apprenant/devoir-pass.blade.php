...
<head>
    ...
    <title>Devoir - {{ $devoir->titre ?? 'Devoir' }} - BJ Academie</title>
...
<body oncontextmenu="return false;">
    ...
    <div id="securityOverlay" class="fixed inset-0 bg-black z-[10000] flex items-center justify-center" style="display: none;">
        <div class="bg-white rounded-xl p-8 max-w-md mx-4 text-center">
            ...
            <p class="text-gray-600 mb-6">Le mode plein écran est obligatoire pour passer ce devoir. Veuillez autoriser le mode plein écran pour continuer.</p>
    ...
    <div class="flex h-screen overflow-hidden {{ $codeUnlocked ? '' : 'locked-interface' }}" id="mainContainer">
        ...
        <aside id="quizSidebar" class="quiz-sidebar flex flex-col h-full" style="{{ $codeUnlocked ? '' : 'display: none;' }}">
            ...
            <div class="bg-gray-800 text-white p-4">
                <div>
                    <h2 class="text-lg font-semibold mb-2">Devoir</h2>
                ...
                <div class="max-w-4xl mx-auto">
                    @if($questions && $questions->count() > 0)
                        ...
                        @if($currentQuestion)
                            ...
                            <div class="border-2 border-gray-300 rounded-lg p-6">
                                <form id="devoirForm" action="{{ route('apprenant.devoir.submit', $devoir->id) }}" method="POST">
                                    @csrf
                                    ...
                                    <div class="flex justify-between items-center pt-6 border-t border-gray-200">
                                        @if($currentQuestionIndex > 0)
                                            <a href="{{ route('apprenant.devoir.passer', ['id' => $devoir->id, 'q' => $currentQuestionIndex - 1]) }}" class="px-6 py-3 border-2 border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 inline-block text-center">
                                                ← Question précédente
                                            </a>
                                        @else
                                            <div></div>
                                        ...
                                        @if($currentQuestionIndex < $totalQuestions - 1)
                                            <a href="{{ route('apprenant.devoir.passer', ['id' => $devoir->id, 'q' => $currentQuestionIndex + 1]) }}" class="px-6 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 inline-block text-center">
                                                Question suivante →
                                            </a>
                                        @else
                                            <button type="button" onclick="submitDevoir(event)" class="px-6 py-3 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 inline-block text-center">
                                                Soumettre le devoir
                                            </button>
                                        @endif
                                    </div>
                    ...
                    @else
                        <div class="text-center py-12">
                            <p class="text-gray-600">Aucune question disponible pour ce devoir.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <script>
        ...
        // Submit devoir
        function submitDevoir(event) {
            event.preventDefault();
            
            if (confirm('Êtes-vous sûr de vouloir soumettre votre devoir ? Vous ne pourrez plus modifier vos réponses.')) {
                document.getElementById('devoirForm').submit();
            }
        }
        ...
        serverCheckInterval = setInterval(() => {
            fetch('{{ route("apprenant.devoir.check-time", $devoir->id) }}', {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                if (data.temps_ecoule) {
                    ...
                    alert('Le temps est écoulé ! Le devoir sera soumis automatiquement.');
                    document.getElementById('devoirForm').submit();
        ...
        function unlockInterface() {
            ...
            fetch('{{ route("apprenant.devoir.unlock", $devoir->id) }}', {
                method: 'POST',
                headers: {
