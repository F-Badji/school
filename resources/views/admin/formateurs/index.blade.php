...
@section('breadcrumb', 'Formateurs')
@section('page-title', 'Gestion des Formateurs')
  ...
  function filterTable(searchTerm) {
    ...
    allRows.forEach(row => {
      if (row.querySelector('td[colspan]')) {
        // C'est la ligne "Aucun formateur"
        return;
      }
    ...
    // Afficher/masquer la ligne "Aucun formateur trouvé"
    ...
    if (emptyRow) {
      ...
      if (term === '') {
        if (visibleCount === 0 && allRows.length === 1) {
          ...
          emptyRow.textContent = 'Aucun formateur enregistré';
        } else {
        ...
        if (visibleCount === 0) {
          emptyRowParent.style.display = '';
          emptyRow.textContent = 'Aucun formateur trouvé pour "' + searchTerm + '"';
        } else {
          emptyRowParent.style.display = 'none';
        }
      }
...
<div class="row">
  <div class="col-12">
    ...
    <div class="card mb-4">
      <div class="card-body">
        <form action="{{ route('admin.formateurs.index') }}" method="GET" class="row g-3">
          <div class="col-md-4">
            <label class="form-label">Spécialité</label>
            <input type="text" name="filiere" class="form-control" placeholder="Filtrer par spécialité" value="{{ request('filiere') }}">
          </div>
          <div class="col-md-3">
          ...
          @if(request()->hasAny(['filiere', 'statut']))
          <div class="col-12">
            <a href="{{ route('admin.formateurs.index') }}" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="card mb-4">
      <div class="card-header pb-0">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <a href="{{ route('admin.formateurs.create') }}" class="btn btn-primary btn-sm mb-0">
            <i class="ni ni-fat-add"></i> Nouveau Formateur
          </a>
        </div>
      </div>
      <div class="card-body px-0 pt-6 pb-2">
          ...
          <table class="table align-items-center mb-0">
            <thead>
              <tr>
                ...
                <th class="text-center text-uppercase text-secondary text-xs font-weight-bold" style="font-size: 0.75rem; color: #344767 !important;">Niveau d'étude assigné</th>
                <th class="text-center text-uppercase text-secondary text-xs font-weight-bold" style="font-size: 0.75rem; color: #344767 !important;">Spécialité</th>
            ...
            <tbody id="formateursTableBody">
              ...
              <tr>
                <td class="text-center">
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->matricule ?? 'N/A' }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->prenom ?? 'N/A' }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->nom ?? 'N/A' }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->email }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->phone ?? 'N/A' }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">{{ $formateur->location ?? 'N/A' }}</p>
                </td>
                <td>
                  <p class="text-xs font-weight-bold mb-0">
                    @if($formateur->nationalite)
                      @php
                        $countryFlags = [
                        ...
                        $flag = $countryFlags[$formateur->nationalite] ?? '';
                      @endphp
                      {{ $flag }} {{ $formateur->nationalite }}
                    @else
                      N/A
                    @endif
                  </p>
                ...
                <td class="align-middle text-center text-sm">
                  @if($formateur->niveau_etude)
                    <span class="badge badge-sm bg-gradient-primary">{{ $formateur->niveau_etude }}</span>
                  @elseif($formateur->classe_id)
                    @php
                      $classeLabels = [
                        'licence_1' => 'Licence 1',
                        'licence_2' => 'Licence 2',
                      ...
                      $classeLabel = $classeLabels[$formateur->classe_id] ?? ucfirst(str_replace('_', ' ', $formateur->classe_id));
                    @endphp
                ...
                <td class="align-middle text-center text-sm">
                  @if($formateur->matieres && $formateur->matieres->count() > 0)
                    <div class="d-flex flex-wrap gap-1 justify-content-center">
                      @foreach($formateur->matieres->take(2) as $matiere)
                        <span class="badge badge-sm bg-gradient-info">
                          {{ Str::limit($matiere->nom_matiere ?? $matiere->nom ?? $matiere->libelle ?? 'Matière', 15) }}
                        </span>
                      @endforeach
                      @if($formateur->matieres->count() > 2)
                        <span class="text-xs text-secondary">+{{ $formateur->matieres->count() - 2 }}</span>
                      @endif
                    </div>
                  @else
                    <span class="text-secondary text-xs font-weight-bold">-</span>
                ...
                <td class="align-middle text-center">
                  <span class="text-secondary text-xs font-weight-bold">{{ $formateur->created_at->format('d/m/Y') }}</span>
                </td>
                <td class="align-middle text-center text-sm">
                ...
                <td class="align-middle">
                  <div class="d-flex gap-2 justify-content-end">
                    <a href="{{ route('admin.formateurs.show', $formateur) }}" class="btn btn-link action-btn p-2 mb-0" data-toggle="tooltip" data-original-title="Voir les détails" style="color: #17a2b8 !important; opacity: 1 !important;">
                    ...
                    <a href="{{ route('admin.formateurs.edit', $formateur) }}" class="btn btn-link action-btn p-2 mb-0" data-toggle="tooltip" data-original-title="Modifier" style="color: #5e72e4 !important; opacity: 1 !important;">
                    ...
                    @else
                      <button type="button" class="btn btn-link action-btn p-2 mb-0 block-btn" data-formateur-id="{{ $formateur->id }}" data-original-title="Bloquer" style="color: #fb6340 !important; opacity: 1 !important;">
                      ...
                      <!-- Modal pour bloquer -->
                      <div class="modal fade" id="blockModal{{ $formateur->id }}" tabindex="-1" role="dialog" aria-labelledby="blockModalLabel{{ $formateur->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="blockModalLabel{{ $formateur->id }}">Bloquer le formateur</h5>
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                              </button>
                            </div>
                            <form action="{{ route('admin.formateurs.toggle-block', $formateur) }}" method="POST">
                              @csrf
                              @method('POST')
                              <input type="hidden" name="action" value="block">
                              <div class="modal-body">
                                <div class="form-group">
                                  <label for="motif_blocage{{ $formateur->id }}">Motif du blocage <span class="text-danger">*</span></label>
                                  <textarea class="form-control" id="motif_blocage{{ $formateur->id }}" name="motif_blocage" rows="4" placeholder="Veuillez saisir le motif du blocage..." required></textarea>
                                  <small class="form-text text-muted">Ce motif sera affiché au formateur lors de sa tentative de connexion.</small>
                                </div>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
                                <button type="submit" class="btn btn-danger">Bloquer le formateur</button>
                              </div>
                            </form>
                          </div>
                        </div>
                    ...
                    <button type="button" 
                            ...
                            data-toggle="modal" 
                            data-target="#deleteConfirmModal{{ $formateur->id }}" 
                    ...
                    @include('components.delete-confirm-modal', [
                      'id' => $formateur->id,
                      'action' => route('admin.formateurs.destroy', $formateur),
              ...
              <tr>
                <td colspan="11" class="text-center py-4">
                  <p class="text-xs text-secondary mb-0">Aucun formateur trouvé</p>
                </td>
              </tr>
              @endforelse
            </tbody>
        ...
        <div class="card-footer px-3 pb-0">
          <div class="d-flex justify-content-between align-items-center">
            <p class="text-xs text-secondary mb-0">Affichage de {{ $formateurs->firstItem() }} à {{ $formateurs->lastItem() }} sur {{ $formateurs->total() }} formateurs</p>
            <div>
              {{ $formateurs->links() }}
            </div>
          </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(forceIconColors, 100);
    setTimeout(forceIconColors, 500);
    ...
    blockButtons.forEach(button => {
      button.addEventListener('click', function(e) {
        ...
        const modal = document.getElementById('blockModal' + formateurId);
        if (modal) {
          ...
          if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
            ...
            openModalVanilla(modal, formateurId);
          }
        }
      });
    });
    
    function openModalVanilla(modal, formateurId) {
      // Afficher le modal
      ...
      // Créer le backdrop
      let backdrop = document.getElementById('modalBackdrop' + formateurId);
      if (!backdrop) {
        backdrop = document.createElement('div');
        backdrop.className = 'modal-backdrop fade show';
        backdrop.id = 'modalBackdrop' + formateurId;
